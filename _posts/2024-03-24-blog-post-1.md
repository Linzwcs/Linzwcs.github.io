...
DP: (1) Want to improve training speed; (2) Don’t want to modify the code too much; (3) There is one machine with multiple GPUs (single machine with multiple cards): In this case, it is recommended to use Data Parallel distributed training.

DDP&FSDP: (1) Want to further improve training speed; (2) Willing to modify the code as needed; (3) There is one machine with multiple GPUs (single machine with multiple cards), or multiple machines (multiple machines with multiple cards), each machine having multiple GPUs: In this case, it is recommended to use Distributed Data Parallel distributed training.
